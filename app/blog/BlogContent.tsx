...
export const BlogContent = ({ posts }: BlogContentProps) => {
  ...
  const [filterValue, setFilterValue] = useState<'survivor' | 'tv' | undefined>(undefined)
        ...
        return (
          <Link href={`blog/post/${post.slug}`} key={idx}>
            ...
            >
              ...
              <p className={paragraphClass + ' !mt-1'}>{formatDate(post.publishDate)}</p>
              <p className={paragraphClass}>{`${post.excerpt}...`}</p>
            </section>
          </Link>
        )
      })}
      {filteredPosts.length === 0 && <h4>{`Uh... there's nothing here... sorry!`}</h4>}
    </>
  )
}
